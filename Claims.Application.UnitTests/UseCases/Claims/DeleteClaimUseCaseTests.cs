...
public class DeleteClaimUseCaseTests
{
    private readonly Fixture _fixture = new();

    public DeleteClaimUseCaseTests()
    {
    ...
    [Fact]
    public async Task ExecuteAsync_Should_DeleteClaimAndWriteAuditOutbox()
    {
        // Arrange
        ...
        var command = _fixture.Build<DeleteClaimCommand>()
            .With(x => x.Id, claimId)
            .With(x => x.HttpMethod, TestConstants.HttpMethods.Delete)
            .Create();

        var sut = _fixture.Create<DeleteClaimUseCase>();
